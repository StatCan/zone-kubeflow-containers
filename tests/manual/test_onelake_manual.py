#!/usr/bin/env python3
"""
OneLake layer — Manual Validation Script
Run this inside a OneLake-enabled Zone Kubeflow notebook or terminal.
...
def test_sdk_packages():
    ...
    print(f"\n  {DIM}Why: the onelake layer adds 'azure-storage-file-datalake' and 'azure-identity'")
    print(f"  in its Docker layer. If these don't import, the entire OneLake")
...
def test_helper_import():
    ...
    print(f"\n  {DIM}Why: the onelake Docker layer copies onelake_utils.py into site-packages. This is")
...
def test_live_connection():
    ...
    print(f"  OneLake API, read, write. If this passes, the OneLake-enabled image works.{RESET}")
    ...
    test_data = f"Zone OneLake validation | {time.strftime('%Y-%m-%d %H:%M:%S')} | onelake layer"
...
def summary():
    ...
    if failed == 0 and skipped == 0:
        print(f"  {GREEN}{BOLD}All {total} checks passed. The OneLake-enabled image is validated.{RESET}\n")
    elif failed == 0:
...
def main():
    ...
    print(f"\n{BOLD}{'═' * 70}{RESET}")
    print(f"{BOLD}  OneLake layer — Validation Suite{RESET}")
    print(f"{BOLD}  {DIM}zone-kubeflow-containers | system-wide onelake layer before mid{RESET}")
