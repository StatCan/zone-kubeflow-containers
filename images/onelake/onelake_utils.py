...
_MOUNT_SCRIPT = Path("/etc/cont-init.d/04-onelake-mount")
...
_MSG = {
    "en": {
        ...
        "mount_unavailable": "OneLake mount helper is not available in this environment.",
        "mount_failed": "OneLake mount failed. Check /tmp/blobfuse2-mount.log for details.",
        "already_mounted": "OneLake is already mounted at {path}. Use remount=True to refresh it.",
    },
    "fr": {
        ...
        "mount_unavailable": "L'assistant de montage OneLake n'est pas disponible dans cet environnement.",
        "mount_failed": "Le montage OneLake a echoue. Verifiez /tmp/blobfuse2-mount.log pour les details.",
        "already_mounted": "OneLake est deja monte a {path}. Utilisez remount=True pour le rafraichir.",
    },
}
...
def _save_config(workspace, lakehouse):
    """Persist the selected workspace/lakehouse for future sessions."""
...
def _write_status(workspace, lakehouse, mounted=None):
    """Write the current OneLake status to ~/.onelake_status."""
    payload = {
        "configured": True,
        "workspace": workspace,
        "lakehouse": lakehouse,
...
def _set_config(workspace, lakehouse):
    """Update env vars, persisted config, and cached clients."""
    global _client, _fs_client
    os.environ["ONELAKE_WORKSPACE"] = workspace
    os.environ["ONELAKE_LAKEHOUSE"] = lakehouse
    try:
        _save_config(workspace, lakehouse)
    except OSError:
        pass
    try:
        _write_status(workspace, lakehouse, mounted=_is_mounted())
    except OSError:
        pass
    _client = None
    _fs_client = None
...
def _unmount():
    """Unmount the local OneLake mount if present."""
    if not _is_mounted():
        return
    ...
    for command in (
        ...
        if result.returncode == 0 or not _is_mounted():
            return
        last_error = result.stderr.strip() or result.stdout.strip() or last_error

    raise RuntimeError(last_error or f"Failed to unmount {MOUNT_PATH}")
...
def connect(workspace, lakehouse):
    ...
    """
    _set_config(workspace, lakehouse)
    print(_msg("switched").format(ws=workspace, lh=lakehouse))
    if _is_mounted():
        print(_msg("mount_warning"))


def mount(workspace=None, lakehouse=None, remount=False):
    """Mount OneLake into ~/onelake for the current or provided config.

    Args:
        workspace: Optional workspace name or GUID. Defaults to current config.
        lakehouse: Optional lakehouse name or GUID. Defaults to current config.
        remount: If True, unmount an existing ~/onelake mount before mounting again.

    Returns:
        True when the mount is available at ~/onelake.
    """
    current_workspace, current_lakehouse = _get_config()
    workspace = workspace or current_workspace
    lakehouse = lakehouse or current_lakehouse
    ...
    if not _MOUNT_SCRIPT.exists():
        raise RuntimeError(_msg("mount_unavailable"))

    if _is_mounted():
        if not remount:
            print(_msg("already_mounted").format(path=MOUNT_PATH))
            return True
        _unmount()
    ...
    if not mounted:
        raise RuntimeError(output or _msg("mount_failed"))

    print(_msg("mounted").format(path=MOUNT_PATH))
    return True
